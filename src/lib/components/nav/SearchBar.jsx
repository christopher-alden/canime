...
export function SearchBar() {
    ...
    function handleSubmit(event) {
      ...
      if (searchQuery.toLowerCase().includes("hentai")) {
        navigate('/Error');
        ...
        const newSearchUrl = `${window.location.origin}${window.location.pathname}?${urlSearchParams.toString()}`;
        window.history.replaceState(null, '', newSearchUrl);
        window.dispatchEvent(new Event('popstate'));
      }
    }    
    
    return (
      <form className="form collapsed" onSubmit={handleSubmit}>
